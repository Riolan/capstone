 ...
 */
 ...
 //constexpr size_t WDMA1_BUFFER_SIZE = 640 * (480 / 4);

 // Declare the buffer and assign it to the custom section mapped to SRAM2
 __attribute__((section(".sram2_buffer_data"))) uint8_t jpeg_img[208000] __ALIGNED(32);
 ...
 static void cisdp_wdma_addr_init(APP_DP_INP_SUBSAMPLE_E subs)
 {
	 ...
	 dbg_printf(DBG_LESS_INFO,"Outside of subs\n");    

	 if(subs == APP_DP_RES_RGB640x480_INP_SUBSAMPLE_1X) {
		 g_jpegautofill_addr = mm_reserve_align(100,0x20);
		 g_wdma1_baseaddr = jpeg_img; ///mm_reserve(640*(480/4)); //640*480/4 g_wdma_buffer; //
		 if(g_wdma1_baseaddr!=0)
			 g_wdma2_baseaddr = g_wdma1_baseaddr;
		 else
			 return ;
		dbg_printf(DBG_LESS_INFO,"g_wdma1_baseaddr = %x, DYNAMIC_ADDRESS, APP_DP_RES_RGB640x480_INP_SUBSAMPLE_1X \n", g_wdma1_baseaddr);    

		 g_wdma3_baseaddr= mm_reserve(921600); //640*480*3
		 dbg_printf(DBG_LESS_INFO,"mm_reserve(921600) returned: 0x%x\n", g_wdma3_baseaddr); // <-- Add this
		 if (g_wdma3_baseaddr == 0) {
			  dbg_printf(DBG_LESS_INFO,"!!! Failed to allocate g_wdma3_baseaddr !!!\n"); // <-- Add this
		 }
	 }
 ...
 uint32_t app_get_shared_sahi_addr()
 {
	 //raw data area BBBBBB/GGGGGG/RRRRRR
	 return jpeg_img;	//return B for use
 }
