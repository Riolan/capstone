...
if (!img || !img->data || !img->size) [[unlikely]]
    ...
    printf("SIZE IS %d, buffer_size ID: %d\r\n\n",size, buffer_size);
    // only reallcate memory when buffer size is not enough
    if (img->size > size) [[unlikely]] {
        
        if ((((size + 2u) / 3u) << 2u) + 1u > 26000) [[unlikely]] {
            if (size > 19200) [[unlikely]] {
                printf("buffer_size: %d is too large, please check the image size\r\n\n", buffer_size);
                return std::string("\"image\": \"\"");
            }
            buffer_size = 26000; 
            // 19,200 is the max size of base64 string otherwise it will be too large to send via UART
            printf("buffer_size: %d is too large, please check the image size\r\n\n", buffer_size);
...
void send_device_id() {
    ...
    const auto& ss{concat_strings("\r{\"type\": 0, \"name\": \"",
                                  ...
                                  std::to_string(EL_OK),
                                  ", \"data\": ",
                                  quoted("STC Grove Vision AI (WE2)"),
                                  "}\n")};
    send_bytes(ss.c_str(), ss.size());
