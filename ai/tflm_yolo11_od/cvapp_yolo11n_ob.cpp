...
#define YOLO11_OB_INPUT_TENSOR_WIDTH   320
#define YOLO11_OB_INPUT_TENSOR_HEIGHT  320
...
// Set up the model and assigns the operations that can occur.
...
// Here is where we are actually going to implement the SAHI calls
// 1280x960 image, we are going to slice it into 320x320 images which we 
// then run inference on.
int cv_yolo11n_ob_run(struct_yolov8_ob_algoResult *algoresult_yolo11n_ob) {
    ...
    if(yolo11n_ob_int_ptr!= nullptr) {
		...
		std::vector<std::vector<detection_cls_yolo11>> all_detections;
		// Create slices
		auto slices = create_slices(
		...
		for (const auto& slice : slices) {
			...
			std::vector<uint8_t> slice_image(
				config.slice_width * config.slice_height * 3
			);
		...
		hx_lib_image_resize_BGR8U3C_to_RGB24_helium((uint8_t*)raw_addr, (uint8_t*)yolo11n_ob_input->data.data,  
			...
			map_slice_coordinates(slice_detections, slice, img_width, img_height);

			all_detections.push_back(slice_detections);
		}
